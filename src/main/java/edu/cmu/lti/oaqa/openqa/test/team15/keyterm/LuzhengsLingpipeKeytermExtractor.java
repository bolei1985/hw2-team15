...
public class LuzhengsLingpipeKeytermExtractor extends AbstractKeytermExtractor {
	 ...
	 * 
	 * @see
	 * edu.cmu.lti.oaqa.cse.basephase.keyterm.AbstractKeytermExtractor#getKeyterms
	 * (java.lang.String)
	 */
		...
		for (String questionPart : questionParts) {
			try {
				...
				while (it.hasNext()) {
					...
					String presentTerm = questionPart.substring(
							presentChunk.start(), presentChunk.end());
					keyterms.add(new Keyterm(presentTerm));
				}
			} catch (ClassNotFoundException e) {
				System.err
						.println("No definition for the class has been found.");
				e.printStackTrace();
				return null;
			} catch (IOException e) {
