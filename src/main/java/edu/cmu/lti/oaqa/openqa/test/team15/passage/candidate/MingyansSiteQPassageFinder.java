...
public class MingyansSiteQPassageFinder implements CandidateFinder {
	...
	public List<PassageCandidate> extractPassages(String docId, String text,
			int startPos, List<Keyterm> keytermList) {
		...
		List<Keyterm> keytermString = new ArrayList<Keyterm>();
		
		keytermString.addAll(keytermList);
		for (Keyterm b : keytermList) {
			if(b.getText().contains(" ")==false)continue;
			String[] a = b.getText().split(" ");
			for (String c : a) {
				Keyterm d = new Keyterm(c);
				keytermString.add(d);
				d.setProbablity((float) (b.getProbability() / 3.0));
			}
		}
		...
		if (sentences.size() <= 3) {
			sentencewindows.add(new PassageSpan(text.substring(0,
					sentences.get(sentences.size())), 0, sentences
					.get(sentences.size()), 0));
		} else {
			for (int i = 0; i < sentences.size() - 3; i++) {
				String window = text.substring(sentences.get(i),
						sentences.get(i + 3));
				logger.debug("*******" + window);
		...
		for (PassageSpan sentence : sentencewindows) {
			...
			for (Keyterm keyterm : keytermString) {
				System.out.println("&&&&&&&&&&&&&&&&" + keyterm.getText());
				Pattern p = Pattern.compile(keyterm.getText());
				Matcher m = p.matcher(sentence.text);

				while (m.find()) {
					PassageSpan match = new PassageSpan(keyterm.getText(),
							m.start(), m.end(), keyterm.getProbability());
			...
			try {
				window = new PassageCandidate(docId, sentence.begin + startPos,
						sentence.end + startPos, (float) score, null);
	...
	protected double getScore(double Score1, int k, int matched_cnt,
